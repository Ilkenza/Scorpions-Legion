...
function Nav() {
  ...
  const content = (
    <div className={`md:hidden items-center flex flex-col bg-bgsiva h-screen justify-evenly transform ${click ? "scale-y-100" : "scale-y-0"} transition-transform duration-300 ease-in-out`}>
      <div className="text-3xl items-center inline-flex w-full flex-col">
        <a href="#" className="cursor-pointer font-bold hover:text-glcrvena my-5 transition-all ease-lenear duration-100 text-bela" onClick={toggleContent}>Pocetna</a>
        <a href="#" className="cursor-pointer font-bold hover:text-glcrvena my-5 transition-all ease-lenear duration-100 text-bela" onClick={toggleContent}>Onama</a>
        <a href="#" className="cursor-pointer font-bold hover:text-glcrvena my-5 transition-all ease-lenear duration-100 text-bela" onClick={toggleContent}>Vlasnici</a>
        <a href="#" className="cursor-pointer font-bold hover:text-glcrvena my-5 transition-all ease-lenear duration-100 text-bela" onClick={toggleContent}>FAQ</a>
      </div>
    </div>
  );

  return (
    <header className={`fixed top-0 w-full z-10 ${navbar ? "bg-[#171717b3] border-b-[0.01rem] border-glcrvena backdrop-blur z-10" : ""}`} style={headerStyle}>
      <nav className="flex items-center justify-evenly w-full font-bold text-xl">
        <a href="#" className="cursor-pointer hover:text-glcrvena text-bela transition-all ease-linear duration-100 hidden md:flex" onClick={toggleContent}>Pocetna</a>
        <a href="#" className="cursor-pointer hover:text-glcrvena text-bela transition-all ease-linear duration-100 hidden md:flex" onClick={toggleContent}>Onama</a>
        <a href="#" className="cursor-pointer hover:text-glcrvena text-bela transition-all ease-linear duration-100 hidden md:flex" onClick={toggleContent}>Logo</a>
        <a href="#" className="cursor-pointer hover:text-glcrvena text-bela transition-all ease-linear duration-100 hidden md:flex" onClick={toggleContent}>Vlasnici</a>
        <a href="#" className="cursor-pointer hover:text-glcrvena text-bela transition-all ease-linear duration-100 hidden md:flex" onClick={toggleContent}>FAQ</a>
        <button className="block md:hidden transition" onClick={toggleContent}>
          {click ? (
            <Icons ikonice="close" stilovi="text-4xl text-glcrvena" />
          ) : (
            <Icons ikonice="menu" stilovi="text-4xl text-glcrvena" />
